...
def check_programs_in_path(programs: list) -> None:
    ...
    for program in programs:
        ...
        if which(program) is None:
            ...
            sys.stderr.write(f"{program} found in the system path.\n")
...
def run_transdecoder_longorfs(fasta_file, map_file, out_dir, min_len, log_file):
    """
    Run TransDecoder.LongOrfs with the given parameters.
    """
...
def find_longest_sequence(sequences):
    ...
    longest_sequences = [item for item in sorted_sequences if len(item[1]) == longest_length]
...
def process_orf_group(transcript_id, orf_dict, transcript_groups):
    ...
    if len(longest_orfs) == 1:
        transcript_header, transcript_seq = find_associated_transcript(longest_orf_header, transcript_groups, transcript_id)
        new_transcripts_dict[transcript_id] = transcript_seq
    else:
        associated_transcripts = []
        for header, seq in longest_orfs:
            transcript_header, transcript_seq = find_associated_transcript(header, transcript_groups, transcript_id)
            associated_transcripts.append((transcript_header, transcript_seq))
        longest_transcript_header, longest_transcript_seq = max(associated_transcripts, key=lambda item: len(item[1]))
        new_transcripts_dict[transcript_id] = longest_transcript_seq

    return new_orfs_dict, new_transcripts_dict

def process_orfs_and_transcripts(orf_fasta, transcript_fasta, output_prefix):
    """
    ...
    for transcript_id, orf_dict in orf_groups.items():
        orfs, transcripts = process_orf_group(transcript_id, orf_dict, transcript_groups)
...
def run_cd_hit_est(threads: int, mem: int, c: float, fasta: str, output: str, log_file: str) -> None:
    """
    Run cd-hit-est with the given parameters.
    """
    cmd = [
        "cd-hit-est",
        "-T", str(threads),
...
def process_cd_hit_clusters(cluster_info_path, name_mapping, output_filename):
    """
...
def run_compacta(threads, bam_dir, output, log_file):
    """
    Run Compacta with the specified parameters.
    """
    # Get list of .bam files in the directory
    bam_files_list = [os.path.join(bam_dir, f) for f in os.listdir(bam_dir) if f.endswith('.bam')]
...
def process_compacta_clusters(compacta_cluster_path, output_file_path):
    """
...
def fasta_header_to_length(fasta_path):
    """
    Read a FASTA file and return a dictionary mapping headers to sequence lengths.
    """
    header_to_length = {}
...
def create_graph_from_clusterings(trinity_path, cdhit_path, compacta_path):
    """
    ...
    for clustering_name, clusters in [('Trinity', trinity_clusters), ('CD-HIT', cdhit_clusters), ('Compacta', compacta_clusters)]:
        add_edges_from_clusters(clusters)
        # Print the status to stderr
        sys.stderr.write(f'Processing {clustering_name} clustering: ')
        sys.stderr.write(f'{G.number_of_nodes()} transcripts in {nx.number_connected_components(G)} clusters.\n')

    return G

###### FUNCTION TO SELECT LONGEST TRANSCRIPTS (FIRST BY ORF LENGTH, THEN TRANSCRIPT LENGTH) PER CLUSTER ######

def find_longest_transcripts(graph, orf_lengths, transcript_lengths):
    """
    ...
    for i, component in enumerate(nx.connected_components(graph)):
        ...
        if len(longest_orf_transcripts) > 1:
            longest_transcript = max(longest_orf_transcripts, key=lambda t: transcript_lengths.get(t, 0))
        else:
